...
# TODO: Add is_valid_query for --query
...
def json_tools(file, add=None, backup=False, print_result=False, query=None, verbose=False, write=False):
    ...
    object = root if not query else __query(root=root, query_list=query.split('.'), verbose=verbose)
    if add:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-q', '--query', help='Query an object in the form of `key.index.key` starting from root, Default = root.')
